...
def init(env, env_name):
    ...
    action spaces
    """
    global replay_buffer, epsilon, is_continuous
    replay_buffer = []
...
def get_network(state_dim, action_dim, hidden_nodes=HIDDEN_NODES):
    ...
    w2 = tf.Variable(tf.truncated_normal([hidden_nodes,24]))
    b2 = tf.Variable(tf.constant(0.1, shape=[24]))
    w3 = tf.Variable(tf.truncated_normal([24,action_dim]))
    b3 = tf.Variable(tf.constant(0.1, shape=[action_dim]))
    h1 = tf.nn.relu(tf.nn.xw_plus_b(state_in, w1, b1))
    h2 = tf.nn.relu(tf.nn.xw_plus_b(h1, w2, b2))
    q_values = tf.nn.xw_plus_b(h2, w3, b3)

    q_selected_action = \
        tf.reduce_sum(tf.multiply(q_values, action_in), reduction_indices=1)
    
#    target_net
    global state_target, q_target, w1_t_update, b1_t_update, w2_t_update, b2_t_update, w3_t_update, b3_t_update
    state_target = tf.placeholder("float", [None, state_dim])
    w1_t = tf.Variable(tf.truncated_normal([state_dim,hidden_nodes]))
    b1_t = tf.Variable(tf.constant(0.1, shape=[hidden_nodes]))
    w2_t = tf.Variable(tf.truncated_normal([hidden_nodes,24]))
    b2_t = tf.Variable(tf.constant(0.1, shape=[24]))
    w3_t = tf.Variable(tf.truncated_normal([24,action_dim]))
    b3_t = tf.Variable(tf.constant(0.1, shape=[action_dim]))
    h1_t = tf.nn.relu(tf.nn.xw_plus_b(state_target, w1_t, b1_t))
    h2_t = tf.nn.relu(tf.nn.xw_plus_b(h1_t, w2_t, b2_t))
    q_target = tf.nn.xw_plus_b(h2_t, w3_t, b3_t)
    
    w1_t_update= w1_t.assign(w1)
    b1_t_update= b1_t.assign(b1)
    w2_t_update= w2_t.assign(w2)
    b2_t_update= b2_t.assign(b2)
    w3_t_update= w3_t.assign(w3)
    b3_t_update= b3_t.assign(b3)
    # TO IMPLEMENT: loss function
    # should only be one line, if target_in is implemented correctly
    loss = tf.reduce_mean(tf.square(q_selected_action - target_in))
...
def update_replay_buffer(replay_buffer, state, action, reward, next_state, done,
    ...
    one_hot_action = np.eye(action_dim, dtype='int32')[action]
    # append to buffer
    replay_buffer.append([state, list(one_hot_action), reward, next_state, done])
...
def get_train_batch(q_values, state_in, minibatch):
    ...
    q_target_batch = q_target.eval(feed_dict={
        state_target: next_state_batch
    })
    batch_index = np.arange(BATCH_SIZE, dtype=np.int32)
    q_next = q_target_batch[batch_index, np.argmax(Q_value_batch, axis = 1)]
...
#            target_val = reward_batch[i] + GAMMA * max(Q_value_batch[i])
            target_val = reward_batch[i] + GAMMA * q_next[i]
...
#           loss, optimise_step_t, train_loss_summary_op_t = get_network(state_dim, action_dim)
    ...
    for episode in range(num_episodes):
        ...
        for step in range(ep_max_steps):
            total_steps += 1
            if step%10 == 0:
                session.run(w1_t_update)
                session.run(b1_t_update)
                session.run(w2_t_update)
                session.run(b2_t_update)
                session.run(w3_t_update)
                session.run(b3_t_update)
